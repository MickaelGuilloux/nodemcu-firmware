...
static int lgpio_trig(lua_State *L)
{
  ...
  else
  {
    ...
    if (old_pin_ref != LUA_NOREF)
      luaL_unref(L, LUA_REGISTRYINDEX, old_pin_ref);
...
static void ICACHE_RAM_ATTR seroutasync_cb(os_param_t p)
{
  ...
  if (serout.index < serout.tablelen)
  {
    ...
    serout.index++;
    if (serout.repeats && serout.index >= serout.tablelen)
    {
      serout.index = 0;
      serout.repeats--;
    }
...
static int lgpio_serout(lua_State *L)
{
  ...
  luaL_argcheck(L, lua_istable(L, 3) &&
                       ((serout.tablelen = lua_objlen(L, 3)) < DELAY_TABLE_MAX_LEN),
                3, "Invalid delay_times");
  ...
  if (is_async)
  { // async version for duration above 15 mSec
  ...
  { // sync version for sub-50 µs resolution & total duration < 15 mSec
    do
    {
